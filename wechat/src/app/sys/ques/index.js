...
class Ques extends React.Component {
  ...
  async componentDidMount() {
    ...
    r.quess.forEach((item,i)=>{ item.key = i+1})
    this.setState({ loading: false, dataSource: r.quess, count:r.quess.length })
  }
  ...
  handleSubmit=async ()=> {
    ...
    this.setState({ loading: true })
    let r = await this.props.mainStore.saveQues(params)
    r.quess.forEach((item,i)=>{ item.key = i+1})
    this.setState({ loading: false, dataSource: r.quess, count:r.quess.length })
    message.info("保存数据成功")
  }
